// This is class based beta irb120 cpp file
#include <ros/ros.h>
...
bool BetaInterfaceClass::triggerService(cwru_srv::simple_int_service_messageRequest& request, cwru_srv::simple_int_service_messageResponse& response)
{
    ...
    cout<<"R: "<<endl;
    cout<<g_R_<<endl;
    g_trigger_ = request.req; //inform "main" that we have a new goal!
    return true;
}
...
void BetaInterfaceClass::stuff_trajectory(Vectorq6x1 p, Vectorq6x1 q, double c, trajectory_msgs::JointTrajectory &new_trajectory) {
    ...
    std::vector<trajectory_msgs::JointTrajectoryPoint> trajectory_points(interpolation_amount);
    
    // new_trajectory.points.clear(); 
    int jointsSum = new_trajectory.joint_names.size();
    int trajPointsSum = trajectory_points.size(); // This trajPointsSum is the sum value of the trajectory points ABBY go thru
    ...
    int interJointAngleVecSum = interJointAngleVec.size(); // This interJointAngleVecSum is the sum value of the trajectory points ABBY go thru except the first points
    ...
    for (int i = 1; i < interJointAngleVecSum; i++){
    ...
    while(ros::ok()) {
        ...
        if (g_trigger_) {
            ...
            if (nsolns>0) {      
                ...
                std::vector<int> weight{1,2,3,5,4,6}; //defining a weight vector
                double sum;
                double minimum = 1e6;
                int bestikSoluNo = 0;
                ...
                new_trajectory.points.clear();
		switch (g_trigger_){	
			case 2: { // don't return to home
                		stuff_trajectory(qvec, g_q_state_, 0.0, new_trajectory);
				break;
			}
			case 1:
			default: { // return to home first 
            ...
            g_trigger_ = 0; // reset the trigger
        }
        sleep_timer_ ->sleep();      
    }
}
