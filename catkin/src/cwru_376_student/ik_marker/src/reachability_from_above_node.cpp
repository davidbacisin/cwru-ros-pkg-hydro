...
int main(int argc, char **argv) {
    ...
    ros::Rate timer(1); // sleep timer for 2Hz repetition rate
    ...
    while (ros::ok()) {
        ...
        for (double z_des = 0.9; z_des>-0.4; z_des-=0.1) {
           ...
           for (double y_des=-1.0; y_des<1.0; y_des+= 0.05) {
                ...
                if (nsolns>0) {
                    desPt.y = y_des; // remember the desired y and and assign to destPt.y
                    desPt.z = z_des; // remember the desired z and and assign to destPt.z
                    ik_solver.get_solns(q6dof_solns);
