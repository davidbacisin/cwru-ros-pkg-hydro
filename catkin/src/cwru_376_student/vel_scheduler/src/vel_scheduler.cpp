...
void haltCallback(const geometry_msgs::Twist& h_rcvd){
    ...
    halt_vel_l_x_ = h_rcvd.twist.linear.x;      
    halt_vel_l_y_ = h_rcvd.twist.linear.y;
    halt_vel_l_z_ = h_rcvd.twist.linear.z;
    halt_vel_a_x_ = h_rcvd.twist.angluar.x;
    halt_vel_a_y_ = h_rcvd.twist.angluar.y;
    halt_vel_a_z_ = h_rcvd.twist.angluar.z;
    // the output below could get annoying; may comment this out, but useful initially for debugging
